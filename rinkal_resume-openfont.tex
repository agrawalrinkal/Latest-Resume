%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Deedy - One Page Two Column Resume
% LaTeX Template
%
% IMPORTANT: THIS TEMPLATE NEEDS TO BE COMPILED WITH XeLaTeX
%
% This template uses several fonts not included with Windows/Linux by
% default. If you get compilation errors saying a font is missing, find the line
% on which the font is used and either change it to a font included with your
% operating system or comment the line out to use the default font.
% 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\documentclass[]{rinkal_resume}
\usepackage{fontawesome}

\begin{document}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%     TITLE NAME
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\namesection{Rinkal}{Agrawal}{ \href{mailto:agrawa55114l.rinkal.us@gmail.com}{agrawal.rinkal.us@gmail.com} \thinspace
| \thinspace St.Paul, MN \thinspace
| \thinspace +1 612.532.7639 \thinspace
| \thinspace
\faGithubAlt \thinspace \thinspace 
\faLinkedin \thinspace \thinspace 
\faMedium \thinspace : {/agrawalrinkal}
}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%     COLUMN ONE
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\begin{minipage}[t]{0.30\textwidth} 

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     EDUCATION
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Education} 

\subsection{University Of Minnesota}
\descript{B.S. in Computer Science}
\small May 2017  Graduate | CGPA: 3.64/4.0 \\
\small Dean's List recipient for all semesters
\sectionsep

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     SKILLS
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Exposure}
\subsection{Languages}
\vspace{1pt}
Java \textbullet{} 
Python \textbullet{}
Typescript \textbullet{}
C++ \textbullet{} \\
C \textbullet{}
Swift \textbullet{} 
JavaScript \textbullet{} 
C\# \textbullet{} \\
HTML5 \textbullet{} 
CSS3 \textbullet{}
SQL \textbullet{}
\sectionsep

\vspace{1pt}
\subsection{Frameworks}
\vspace{1pt}
Angular 6 \textbullet{}
Node.js \textbullet{}
PyTorch \textbullet{} \\
Xcode \textbullet{}
Express.js \textbullet{}
Bootstrap4 \textbullet{}
MongoDB \textbullet{}
React \textbullet{}
.Net \textbullet{}
\sectionsep

\vspace{1pt}
\subsection{Strong Knowledge}
\vspace{1pt}
Micro services Architecture \textbullet{}
Git \textbullet{} \\
REST \textbullet{} 
Product Ownership \textbullet{} 
NPM \textbullet{} \\
Unit Testing \textbullet{}
Agile Methodology \textbullet{} \\
Tomcat \textbullet{}
Apache-Storm \textbullet{}
Maven \textbullet{}
%Eclipse \textbullet{}
\sectionsep

\vspace{1pt}
\subsection{Working Knowledge}
\vspace{1pt}
Neural Networks(MLP, CNN, RNN, \\ LSTM)  \textbullet{}
Redis \textbullet{}
Flask \textbullet{}
RabbitMQ \textbullet{} \\
AWS \textbullet{}
Zookeeper \textbullet{}
Firebase \textbullet{} 
MVC \textbullet{} \\
MVVM \textbullet{} 
PHP \textbullet{} 
Multi-threading \textbullet{} 
\sectionsep

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     RESEARCH
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Research}
\runsubsection{Undergrad Researcher} \\
\location{U of M Software Engineering Center Aug '16 – Dec '16}
% \vspace{\topsep} % Hacky fix for awkward extra vertical space

% Fix to horizontally align the multi-line \textbullet{} and adds a horizontal space to the second line
\newlength\boxwid
\settowidth{\boxwid}{\indent\hspace{0.5cm}}

% Adds the horizontal space to the bullet point
% \begin{tightemize}
\hspace{0.2cm} \hangindent=\boxwid 
\textbullet{} Worked on a NASA project under Prof. Sanjai Rayadurgam to display the autonomous applications of a quadcopter drone. \\
\hspace{0.2cm} \hangindent=\boxwid 
\textbullet{} Displayed the autonomous application by installing a tweaked NASA octocopter software on a pixhawk hardware. \\
% \end{tightemize}
\smallsectionsep

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     COURSEWORK
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Coursework}
\textbullet{} Advance Algo and Data structures \\
\textbullet{} Intro to Deep Learning @Udacity\\
\textbullet{} Artificial Intelligence \\
\textbullet{} Software Design and Development \\
\textbullet{} Internet Programming \\
\smallsectionsep

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     Intrest
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Interests}
\textbullet{} Mentoring students w/ CoderDojo \\
\textbullet{} Networking events and webinars \\
\textbullet{} Reading and writing blogs \\
\textbullet{} Financial world \\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%     COLUMN TWO
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\end{minipage} 
\hfill
\begin{minipage}[t]{0.69\textwidth} 

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     EXPERIENCE
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Experience}

\runsubsection{Software Engineer ii }\hfill\  \location{Nov '18 – Present} 
\location{Software Engineer I \hfill June '17 - Nov '18}  
\location{CenturyLink}
\vspace{\topsep} % Hacky fix for awkward extra vertical space
\begin{tightemize}
\item Built a data-driven application using Angular6 on node.js and Java to provide a user interface for multiple inventories, leading to 40\% fewer error tickets. As a product owner, directed features, worked with users to gather requirements and feedback. Application was developed with continuous test-releases.
\item Implemented an automation solution in Java for SD-WAN technology by closely working with different business units, resulting a decrease in device activation time from 30+ minutes to just over 4 minutes.
\item Built a server-side routing engine using JavaScript and JSON on express.js to route the incoming traffic based on the request, resulting in over 20\% decrease in client load.
\item Increased the legacy server engine efficiency by 4 folds, by rewriting the algorithm using SNMP technology and batching.
\end{tightemize}
\smallsectionsep

\runsubsection{Software Developer Intern} \\
\location{Design Ready Controls Inc. \hfill  June '16 – June '17}
\begin{tightemize}
\item Worked on windows desktop applications using C\# and .Net that helped electrical engineers better understand and faster design the customized control panels, reducing the designs delivery time by 30\%.
\item Designed and worked on the UI for the Android app that let users visualize the field sensors data in real time using AWS.
\end{tightemize}
\smallsectionsep

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     PROJECTS
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Projects}
\runsubsection{Eureka}
\descript{| Monitor for team's performance over time} 
\location{Independent @work, 9 weeks  \hfill | Angular6, Java}
\begin{tightemize}
\item Designed and built the app to let team members post their project successes.
\item Lets other people within the company view posted achievements and other project related statistics from an elegant dashboard.
\end{tightemize}
\smallsectionsep

\runsubsection{Style Transfer}
\descript{| CNN to transfer image style} 
\location{Independent, 2 weeks  \hfill | Python, PyTorch}
\begin{tightemize}
\item Designed a Convolutional Neural Network using a pre-trained VGG19 model to transfer the style of one image onto the content of another image.
\item Built the model to extract content and style features for passed in images and later used losses to achieve the desired target image.
\end{tightemize}
\smallsectionsep

\runsubsection{Tweetalizer}
\descript{| Real-time map visualization showing top tweets} 
\location{Independent, 2 weeks \hfill  | Java, Python, Apache-storm}
\begin{tightemize}
\item Built a real-time map visualization using geocode showing top tweets and hashtags on the US map based on the number of likes and re-tweets.
\item Integrated with multiple open-sourced bolts to add features.
\end{tightemize}
\smallsectionsep

\runsubsection{Envoy}
\descript{| Web app for hearing-impaired students} 
\location{Non-Academic, 13 weeks, team of 4  \hfill | React, Java}
\begin{tightemize}
\item Designed to conduct instructor-led online classes for hearing impaired students by implementing video messaging channel to help improve their university experience.
\item Worked with potential hearing-impaired students to understand their problems and collect requirements.
\end{tightemize}
\smallsectionsep

\runsubsection{Translator}
\descript{| CDAL to C++ Code compiler} 
\location{Academic, 15 weeks, team of 2 \hfill  | C++}
\begin{tightemize}
\item Implemented Lexer, Parser and Abstract Syntax Tree and Generator to covert HLL code written in 'Climate Data Analysis Language' to C++. Extensively used testing.
% \item  Built a compiler to translate 'Climate Data Analysis Language' code into C++ code.
% \item Extensively used unit testing to verify conversion correctness.
\end{tightemize}
\smallsectionsep

\end{minipage} 
\end{document}  \documentclass[]{article}
